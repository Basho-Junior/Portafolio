...
import Practica6.Entidades.*;
import Practica6.Services.*;
...
public class CrudTradicionalControlador extends Base {
    ...
    public void aplicarRutas(){
        ...
        app.get("/", ctx -> {
            Carrito carro = ctx.sessionAttribute("carrito");
            List<Producto> productos = ServiceProducto.getInstance().findProd(0, 10);
        ...
        app.post("/comprar", ctx -> {
            ...
            if(temp == null){
                temp = ServiceProducto.getInstance().find(ctx.formParamAsClass("id", Integer.class).get());
        ...
        app.get("/comprar/{id}", ctx -> {
            ...
            Carrito carro = ctx.sessionAttribute("carrito");
            List<Producto> productos = ServiceProducto.getInstance().findProd(pos, pos+10);
            Map<String, Object> modelo = new HashMap<>();
            List<String> paginas = getPaginas();
            if( ctx.cookie("usuario") == null || ctx.cookie("mist") == null) {
                if(ctx.sessionAttribute("usuario") == null){
        ...
        app.get("/ventas", ctx -> {
            ...
            Carrito carro = ctx.sessionAttribute("carrito");
            List<Venta> ventas = ServiceVenta.getInstance().getVentas();
            Map<String, Object> modelo = new HashMap<>();
        ...
        app.get("/remover/{id}", ctx -> {
            ServiceProducto.getInstance().deleteProducto(ctx.pathParamAsClass("id",Integer.class).get());
            ctx.redirect("/productos");
        });
        ...
        app.get("/editar/{id}", ctx -> {
            Producto temp = ServiceProducto.getInstance().find(ctx.pathParamAsClass("id", Integer.class).get());
            Map<String, Object> modelo = new HashMap<>();
        ...
        app.post("/editar/{id}", ctx -> {
            ...
            ServiceProducto.getInstance().edit(temp);
        ...
        app.post("/procesar",ctx -> {
            ...
            List<Comprado> lista = ServiceComprado.getInstance().convertProd(carro.productos,venta.getId());
            venta.setListaProductos(lista);
            service.addVentas(venta);
            ServiceVenta.getInstance().create(venta);
        ...
        app.get("/ver/{id}",ctx -> {
            ...
            Producto temp = ServiceProducto.getInstance().find(Integer.parseInt(id));
