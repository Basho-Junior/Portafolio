...
%Obtener los planetas en donde el día es más corto que en la tierra, solo si los planetas no son gaseosos.
dias_corto(Planeta):-rotacion(tierra,DiaTierra),rotacion(Planeta,DiaPlaneta),planeta(Planeta,L,_,_),not(pertenece(gaseoso,L)),DiaTierra > DiaPlaneta.
