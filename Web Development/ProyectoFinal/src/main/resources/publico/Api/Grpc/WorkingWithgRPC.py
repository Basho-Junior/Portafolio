...
cliente = EnlaceRn_pb2_grpc.EnlaceRnStub(channel)

def consultas(cliente):
    menu1 = '\nCLIENTE ACORTADOR EN gRPC\n 1 - LOGIN\n 2- SEGUIR SIENDO ANONIMO \n 3- SALIR'
    res = 0
...
def autentificar(cliente):
    ...
    while(aux):
        ...
        res = cliente.autentificacion(EnlaceRn_pb2.usuarioRequest(usuario = us,contrasenia = pas))
...
def acortar(cliente):
    url = input('\nURL A ACORTAR-> ')
    res = cliente.makeLink(EnlaceRn_pb2.linkRequest(enlace = url, usuario = user))
...
def listar(cliente):
    if user == 'ANONIMO':
        ...
        res = cliente.getLinks(EnlaceRn_pb2.enlace(user = user)).enlace
...
def mostrar(cliente):
    id = int(input('\nID DEL ENLACE-> '))
    if(id > 0):
        res = cliente.getLink(EnlaceRn_pb2.visitantesRequest(ide = id))
        print('\n\n----------------RESULTADO----------------\n')
        print("ID: " + str(res.idEnlace))
        print('URL Original: '+  res.URL)
        print('URL Acortada: ' + res.URLAcostarda)
        ...
        clientes = cliente.getVisitantes(EnlaceRn_pb2.visitantesRequest(ide = id))
