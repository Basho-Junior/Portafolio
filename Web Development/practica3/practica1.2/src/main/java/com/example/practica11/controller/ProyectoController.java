...
import com.example.practica11.entidades.repositorios.MockProyectoRepo;
...
public class ProyectoController {
    ...
    @Autowired
    private MockProyectoRepo mockProyectoRepo;
    ...
    public String dashboard(Model model) {
        Iterable<MockProyecto> proyectos = null;
        ...
        model.addAttribute("titulo", "Practica 1.1 " + puerto);
    ...
    public RedirectView newProyect() {
        MockProyecto proyecto = new MockProyecto();
    ...
    public String modify(Model model, @PathVariable("id") String id) {
        MockProyecto proyecto = mockProyectoRepo.findById(id).orElse(null);
        ...
        throw new ResponseStatusException(
                HttpStatus.FORBIDDEN, "No tiene permiso para ver este proyecto."
        );
    }
    ...
    public RedirectView addEndpoint(HttpServletRequest request, @PathVariable("id") String id) {
        ...
        if (proyecto != null) {
            ...
            MockEndpointPK endpointID = new MockEndpointPK(id, MockEndpointMethods.valueOf(parameterMap.get("getRadio")[0]), path);
            if (mockEndpointRepo.existsById(endpointID)) {
                ...
                if (endpoint.getJwtRequired()) {
                    endpoint.setJwtToken(generarToken(authService.getUserAuthenticated(),
...
//                proyecto.getEndpoints().add(endpoint);
...
//                mockProyectoRepo.save(proyecto);
    ...
    public String addWithoutProject(HttpServletRequest request) {
        MockProyecto proyectoD = null;// = mockProyectoRepo.findById(id).orElse(null);
        Map<String, String[]> parameterMap = request.getParameterMap();
        String id = "";
        if (parameterMap.get("proyecto")[0].equalsIgnoreCase("0default")) {
            // Tomar primer proyecto o crear proyecto si no existen.
            List<MockProyecto> proyectosUsuario =
                    mockProyectoRepo.findAllByUsuarioId(authService.getUserAuthenticated().getUsername());
            if (proyectosUsuario.size() > 0) {
                // Por defecto enviar al primero
                proyectoD = proyectosUsuario.get(0);
            } else {
                // Si no hay proyectos, entonces crear uno para poder insertar los datos ahi .
                MockProyecto nuevoProyecto = new MockProyecto();
                nuevoProyecto.setUsuarioId(authService.getUserAuthenticated().getUsername());
                mockProyectoRepo.save(nuevoProyecto);
                // Buscar nuevamente los proyectos de este usuario:
                proyectosUsuario =
                        mockProyectoRepo.findAllByUsuarioId(authService.getUserAuthenticated().getUsername());
                // Por defecto enviar al primero
                proyectoD = proyectosUsuario.get(0);
...
//                mockProyectoRepo.save(proyecto);
    ...
    public RedirectView deleteEndpoint(@PathVariable("id") String id, @PathVariable("method") String method, @PathVariable("path") String path) {
        ...
        mockEndpointRepo.deleteById(new MockEndpointPK(id, MockEndpointMethods.valueOf(method), path));
        return new RedirectView("/proyecto/p/" + id);
    }
     ...
     * @return
     */
    private String generarToken(Usuario usuario, Date expirationDate) {
